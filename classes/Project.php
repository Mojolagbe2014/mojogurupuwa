    ...
    public function fetchForJQDT($draw, $totalData, $totalFiltered, $customSql="", $column="*", $condition="", $sort="id"){
        ...
        if(count($data)>0){
            foreach($data as $r){ 
                ...
                $fetProjectFeat = 'icon-eye-close'; $fetProjectFeatCol = 'btn-warning'; $fetProjectFeatTit = "Make Featured Project";
                if($r['status'] == 1){  $fetProjectStat = 'icon-check'; $fetProjectRolCol = 'btn-success'; $fetProjectRolTit = "De-activate Project";}
                if($r['featured'] == 1){  $fetProjectFeat = 'icon-eye-open'; $fetProjectFeatCol = 'btn-success'; $fetProjectFeatTit = "Remove Featured Project";}
